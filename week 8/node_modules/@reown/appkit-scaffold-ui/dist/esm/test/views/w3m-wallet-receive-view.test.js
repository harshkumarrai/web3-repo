...
import { AccountController, AssetUtil, ChainController, CoreHelperUtil, RouterController, SnackController } from '@reown/appkit-controllers';
...
const mockChainControllerState = {
    ...
    activeCaipAddress: `eip155:1:${mockAddress}`,
    activeChain: 'eip155',
    chains: new Map([
        [
            'eip155',
...
describe('W3mWalletReceiveView', () => {
    beforeEach(() => {
        vi.spyOn(AccountController, 'state', 'get').mockReturnValue(mockAccountControllerState);
    ...
    it('should render with address and QR code', async () => {
        const element = await fixture(html `<w3m-wallet-receive-view></w3m-wallet-receive-view>`);
    ...
    it('should copy address on button click', async () => {
        const element = await fixture(html `<w3m-wallet-receive-view></w3m-wallet-receive-view>`);
        await element.updateComplete;
        const copyButton = element.shadowRoot?.querySelector('[data-testid="receive-address-copy-button"]');
...
//# sourceMappingURL=w3m-wallet-receive-view.test.js.map