...
import { ConnectionController, RouterController, SendController, SwapController } from '@reown/appkit-controllers';
...
describe('W3mWalletSendView', () => {
    beforeEach(() => {
        ...
        vi.spyOn(ConnectionController, 'getEnsAddress').mockImplementation((ensName) => {
    ...
    it('should show invalid address message for incorrect address and persist when input cleared', async () => {
        ...
        expect(button?.disabled).to.be.true;
        const addressInput = element.shadowRoot?.querySelector('w3m-input-address');
        addressInput.click();
        const textarea = addressInput.shadowRoot?.querySelector('textarea');
        textarea.value = '';
