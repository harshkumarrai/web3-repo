...
__export(index_exports, {
  ...
  GeminiStorage: () => GeminiStorage,
  GeminiWallet: () => GeminiWallet,
  GeminiWalletProvider: () => GeminiWalletProvider,
  POPUP_HEIGHT: () => POPUP_HEIGHT,
  POPUP_WIDTH: () => POPUP_WIDTH,
  PlatformType: () => PlatformType,
  ProviderEventEmitter: () => ProviderEventEmitter,
  SDK_BACKEND_URL: () => SDK_BACKEND_URL,
  SDK_VERSION: () => SDK_VERSION,
  STORAGE_CALL_BATCHES_KEY: () => STORAGE_CALL_BATCHES_KEY,
  STORAGE_ETH_ACCOUNTS_KEY: () => STORAGE_ETH_ACCOUNTS_KEY,
  STORAGE_ETH_ACTIVE_CHAIN_KEY: () => STORAGE_ETH_ACTIVE_CHAIN_KEY,
  STORAGE_PASSKEY_CREDENTIAL_KEY: () => STORAGE_PASSKEY_CREDENTIAL_KEY,
  STORAGE_PRESERVED_PASSKEY_CREDENTIALS_KEY: () => STORAGE_PRESERVED_PASSKEY_CREDENTIALS_KEY,
  STORAGE_SETTINGS_KEY: () => STORAGE_SETTINGS_KEY,
  STORAGE_SMART_ACCOUNT_KEY: () => STORAGE_SMART_ACCOUNT_KEY,
  STORAGE_WC_REQUESTS_KEY: () => STORAGE_WC_REQUESTS_KEY,
  base64ToHex: () => base64ToHex,
  bufferToBase64URLString: () => bufferToBase64URLString,
  calculateV1Address: () => calculateV1Address,
  calculateWalletAddress: () => calculateWalletAddress,
  closePopup: () => closePopup,
  convertSendValuesToBigInt: () => convertSendValuesToBigInt,
  decodeBase64: () => decodeBase64,
  encodeBase64: () => encodeBase64,
  fetchRpcRequest: () => fetchRpcRequest,
  generateAuthenticatorIdHash: () => generateAuthenticatorIdHash,
  hexStringFromNumber: () => hexStringFromNumber,
  isChainSupportedByGeminiSw: () => isChainSupportedByGeminiSw,
  openPopup: () => openPopup,
  reverseResolveEns: () => reverseResolveEns,
  safeJsonStringify: () => safeJsonStringify,
  utf8StringToBuffer: () => utf8StringToBuffer,
  validateRpcRequestArgs: () => validateRpcRequestArgs,
  validateWebAuthnKey: () => validateWebAuthnKey
});
...
function getDefaultRpcUrl(chainId) {
  const chainMap = {
    [import_chains.mainnet.id]: import_chains.mainnet.rpcUrls.default.http[0],
    [import_chains.arbitrum.id]: import_chains.arbitrum.rpcUrls.default.http[0],
    [import_chains.optimism.id]: import_chains.optimism.rpcUrls.default.http[0],
    [import_chains.base.id]: import_chains.base.rpcUrls.default.http[0],
    [import_chains.polygon.id]: import_chains.polygon.rpcUrls.default.http[0],
    [import_chains.sepolia.id]: import_chains.sepolia.rpcUrls.default.http[0],
    [import_chains.arbitrumSepolia.id]: import_chains.arbitrumSepolia.rpcUrls.default.http[0],
    [import_chains.optimismSepolia.id]: import_chains.optimismSepolia.rpcUrls.default.http[0],
    [import_chains.baseSepolia.id]: import_chains.baseSepolia.rpcUrls.default.http[0],
    [import_chains.polygonAmoy.id]: import_chains.polygonAmoy.rpcUrls.default.http[0]
  };
...
var ProviderEventEmitter = class extends import_eventemitter3.EventEmitter {
};
...
function generateAuthenticatorIdHash(credentialId) {
  ...
  return (0, import_viem.keccak256)(bytes);
}
...
function calculateAddressInternal(params) {
  ...
  const validatorInitData = (0, import_viem.encodeAbiParameters)(
    [
      {
        components: [
  ...
  const bootstrapCall = (0, import_viem.encodeFunctionData)({
    abi: [
      {
        inputs: [
...
function predictProxyAddress(implementation, salt, initData, deployer) {
  const initializeCall = (0, import_viem.encodeFunctionData)({
    abi: [
      {
  ...
  const constructorArgs = (0, import_viem.encodeAbiParameters)(
...
var openPopup = (url) => {
  ...
  popup?.focus();
  if (!popup) {
    throw import_rpc_errors.rpcErrors.internal("Pop up window failed to open");
  }
  return popup;
};
  ...
  constructor({ appMetadata, onDisconnectCallback }) {
    ...
    this.onRequestCancelled = () => {
      ...
      this.listeners.forEach(({ reject }, listener) => {
        reject(import_rpc_errors2.providerErrors.userRejectedRequest());
        window.removeEventListener("message", listener);
      });
    ...
    this.waitForPopupLoaded = () => {
      ...
      return this.onMessage(
        ...
        if (!this.popup) throw import_rpc_errors2.rpcErrors.internal();
        return this.popup;
      });
    };
...
var import_rpc_errors4 = require("@metamask/rpc-errors");
...
var import_rpc_errors3 = require("@metamask/rpc-errors");
...
function validateRpcRequestArgs(args) {
  if (!args || typeof args !== "object" || Array.isArray(args)) {
    throw import_rpc_errors3.rpcErrors.invalidParams({
    ...
    try {
      ...
      switch (args.method) {
        ...
        case "wallet_sign":
          ...
          if (response.error) {
            throw import_rpc_errors4.rpcErrors.transactionRejected(response.error);
          } else {
        ...
        case "wallet_switchEthereumChain": {
          ...
          if (Array.isArray(rawParams) && rawParams[0]?.chainId) {
            ...
            throw import_rpc_errors4.rpcErrors.invalidParams(
              "Invalid chain id argument. Expected [{ chainId: hex_string }] or { id: number }."
            );
          }
          response = await this.wallet.switchChain({ id: chainId });
          if (response) {
            throw import_rpc_errors4.providerErrors.custom({ code: 4902, message: response });
          }
        ...
        case "wallet_grantPermissions":
          throw import_rpc_errors4.rpcErrors.methodNotSupported("Not yet implemented.");
        // not supported
        case "eth_sign":
        case "eth_coinbase":
        case "wallet_addEthereumChain":
          throw import_rpc_errors4.rpcErrors.methodNotSupported();
      ...
      const { code } = error;
      if (code === import_rpc_errors4.errorCodes.provider.unauthorized) this.disconnect();
      return Promise.reject((0, import_rpc_errors4.serializeError)(error));
    }
  }
