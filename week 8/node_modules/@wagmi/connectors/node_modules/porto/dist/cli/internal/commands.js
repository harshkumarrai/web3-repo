...
export async function createAccount(_, args) {
    ...
    if (adminKey) {
        try {
            ...
            tempKeyFile = path.join(tmpDir, `admin-key-${Date.now()}-${Math.random().toString(36).slice(2)}.key`);
            fs.writeFileSync(tempKeyFile, adminKey.privateKey(), { mode: 0o600 });
        }
        catch { }
    }
    let shouldPrintKeyOnExit = Boolean(adminKey);
    function cleanup(label, err) {
        if (!shouldPrintKeyOnExit)
        ...
        if (adminKey) {
            if (tempKeyFile) {
                prompts.log.info(`Admin key saved securely to temporary file: ${tempKeyFile}`);
            }
            else {
                prompts.log.warn('Admin key could not be saved to a temporary file. Please rerun to generate a new key.');
            }
        }
    ...
    const onUncaught = (error) => cleanup('uncaughtException', error);
    const onUnhandled = (reason) => cleanup('unhandledRejection', reason);
    const onExit = () => {
        if (!shouldPrintKeyOnExit)
            return;
        if (adminKey) {
            if (tempKeyFile) {
                prompts.log.info(`Admin key saved securely to temporary file:\n${tempKeyFile}`);
            }
            else {
                prompts.log.warn('Admin key could not be saved to a temporary file. Please rerun to generate a new key.');
            }
    ...
    if (adminKey)
        Dialog.messenger.registerPublicKey(adminKey.publicKey);
    ...
    if (adminKey) {
        ...
        if (reveal)
            ...
            const keyFile = path.join(import.meta.dirname, `${accounts[0].address}.key`);
