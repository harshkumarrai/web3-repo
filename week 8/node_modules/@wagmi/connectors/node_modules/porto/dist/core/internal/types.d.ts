 ...
 * ```
 */
export type FixedArray<type, count extends number, result extends readonly type[] = []> = result['length'] extends count ? result : FixedArray<type, count, readonly [...result, type]>;
/**
...
export type DeepReadonly<T> = T extends Builtin ? T : IsUnknown<T> extends true ? T : T extends object ? Readonly<{
    [K in keyof T]: T[K] extends Primitive ? T[K] : DeepReadonly<T[K]>;
}> : T extends Array<infer U> ? U extends Primitive ? Readonly<Array<U>> : Readonly<Array<DeepReadonly<U>>> : never;
export {};
